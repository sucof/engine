...
function filesizeTooBigPopUp() {
    showErrorPopUp(
        ...
        getImageAsDiv("images/too-big.png", "Filsize too image") + getTextAsDiv('The file you are trying to submit is too big for being analyzed</br>We are sorry! :(</br>PD: We are working on it!'),
        'Accept',
...
function fileIsEmptyPopUp() {
    showErrorPopUp(
        'The file is empty',
        'z-dialog-vertical-center',
        getImageAsDiv("images/file-empty.png", "Filesize empty") + getTextAsDiv('We have detected that there is no useful data in the file you are trying to upload.</br>We are sorry! </br>:('),
        'Accept',
        undefined, OK_ONLY_DIALOG);
    return;
}
...
function hashSignatureCheckFailedPopUp() {
    showErrorPopUp(
        'Connection error',
        'z-dialog-vertical-center',
        getImageAsDiv("images/network-connection-error-icon.png", "Fingerprint image") + getTextAsDiv('It seems that something is blocking the Internet connection because we could not connect. Please check your connection cable and be sure that Firewall rules are not blocking this connection.'),
        'Accept',
        undefined, OK_ONLY_DIALOG);
}
...
function noServerConnectionPopUp() {
    showErrorPopUp(
        'Connection error',
        'z-dialog-vertical-center',
        getImageAsDiv("images/driver-offline.png", "server down image", 30) + getTextAsDiv('It seems we have some difficulties connecting with the server right now.</br>Please, try it later.'),
        'Accept',
        undefined, OK_ONLY_DIALOG);
}
...
function showBetaAccessErrorPopUp() {
    showErrorPopUp(
        ...
        'z-dialog-vertical-center',
        getImageAsDiv("images/ninja.png", "Ninja attack image") + getTextAsDiv('We are under ninja\'s attack right now and we can not fight against them and give you access at the same time. So we think, the best for you, is to try it later...when they are gone!!'),
        'Accept',
        undefined, OK_ONLY_DIALOG);
}
...
function showFileNotApkPopUp(name) {
    showInfoPopUp(
        ...
        'z-dialog-vertical-center',
        '<div class="row"><div class="col-md-6 col-xs-6" style="width: 157px;">' + getImageAsDiv("images/andy-error.jpg", "Error Andy icon", 120) + '</div><div class="col-md-6 col-xs-6">' + getTextAsDiv('FOCUS!! </br> <tt style="font-family: courier new;">' + cleanHtml(name) + '</tt> </br>Is not a valid <i>Android application.</i>') + '</div></div></div>',
        'Accept',
        undefined, OK_ONLY_DIALOG);
}
